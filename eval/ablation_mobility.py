...
def run_mobility_ablation(config_path: str = "configs/default_config.yaml"):
    ...
    for v in velocities_ms:
        ...
        for seed in seeds:
            set_seed(seed)
            test_dict = generate_isac_samples(config_v, num_samples=30, seed=seed)
        ...
        print(f"Velocity: {v:5.1f} m/s ({v*3.6:5.1f} km/h) | LMMSE NMSE: {np.mean(seed_results['LMMSE_NMSE']):.2f} dB | Trans NMSE: {np.mean(seed_results['Trans_NMSE']):.2f} dB | Mamba NMSE: {np.mean(seed_results['Mamba_NMSE']):.2f} dB")
