...
class MambaISAC(nn.Module):
    ...
    Uses residual channel correction: H_c_hat = Y_obs + Delta_H.
    """
    ...
    def forward(self, Y_obs: torch.Tensor, bidirectional: bool = True) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        z_rep = self.backbone(z_embed, bidirectional=bidirectional)
        
        # Residual channel estimation: H_c_hat = Y_obs + residual_correction
