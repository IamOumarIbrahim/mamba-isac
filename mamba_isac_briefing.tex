...
\item An open-source, reproducible code repository containing synthetic channel generators, PyTorch GPU training scripts, evaluation pipelines, and figure generation tools.
\end{itemize}
...
\midrule
LMMSE & -2.73 $\pm$ 0.24 & 9.70 $\pm$ 1.38 & 2.62e5 & 0.65 $\pm$ 0.03 \\
Transformer & -8.74 $\pm$ 0.09 & 6.18 $\pm$ 0.66 & 1.41e7 & 2.22 $\pm$ 0.10 \\
\textbf{Mamba-ISAC} & \textbf{-9.52 $\pm$ 0.02} & \textbf{7.26 $\pm$ 1.85} & 1.55e7 & 17.50 $\pm$ 0.45 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Benchmark Comparison}
Table~\ref{tab:main_results} summarizes performance across all three estimators evaluated on an identical held-out test split under $15\text{ dB}$ SNR. Mamba-ISAC achieves communication channel NMSE of $-9.52\text{ dB}$, outperforming the parameter-matched Transformer baseline ($-8.74\text{ dB}$) by $0.78\text{ dB}$ and classical LMMSE ($-2.73\text{ dB}$) by $6.79\text{ dB}$.

\subsection{SNR and Mobility Sweeps}
Under SNR sweeps from $-10\text{ dB}$ to $+30\text{ dB}$, Mamba-ISAC reaches $-13.34\text{ dB}$ NMSE at $30\text{ dB}$ SNR compared to Transformer's $-11.65\text{ dB}$. High-mobility evaluations up to $120\text{ m/s}$ ($432\text{ km/h}$) confirm that state-space temporal modeling maintains estimation accuracy under high Doppler shifts.

\section{Discussion and Limitations}

While Mamba-ISAC demonstrates linear sequence complexity and superior joint range/CSI estimation accuracy, PyTorch sequential loop overhead introduces wall-clock execution latency compared to PyTorch's native C++ MHA implementation. Building custom CUDA selective-scan kernels (`mamba-ssm`) directly eliminates this Python interpreter loop overhead.
...
This paper introduced Mamba-ISAC, a selective state-space network architecture for joint channel estimation in 6G OFDM ISAC systems. By sharing a selective state-space backbone between communication CSI estimation and radar parameter regression, Mamba-ISAC achieves joint estimation accuracy superior to Transformer baselines ($-9.52\text{ dB}$ vs $-8.74\text{ dB}$ NMSE) while maintaining linear sequence length scaling $O(T)$. Future work will extend the framework to multi-target radar clutter environments and CUDA-accelerated hardware implementations.
